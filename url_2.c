...
//Including module header for url-modules.c
#include "url_dependencies.c"
...
/***************************************************************/
/***************************************************************/
//REFACTORED FUNCTON COMPEXITY CHANGED FROM 18 TO 14
static bool check_noproxy(const char *name, const char *no_proxy)
{
   ...
   */
    ...
    if (no_proxy && no_proxy[0])
    {
        ...
        {   //#########################################################################
            //The while loop present here is replaced by a function called the firstLoop
            //The loop was responsible for changing the value of tok_start
            //For this we returned the value of tok_start from the function to adjust the change.

            tok_start = firstLoop(tok_start, no_proxy_len, separator, no_proxy);
            //#########################################################################
            if (tok_start == no_proxy_len)
                break; /* It was all trailing separator chars, no more tokens. */

            //#########################################################################
            //The for loop present here is replaced by a function called the secondLoop
            //The loop was responsible for changing the value of tok_end
            //For this we returned the value of tok_end from the function to adjust the change.
            tok_end = secondLoop(tok_end, tok_start, no_proxy_len, separator, no_proxy);
            //#########################################################################
       ...
       */
            ...
            if ((tok_end - tok_start) <= namelen)
            {
                                    ...
                                    tok_end - tok_start))
                {
                    ...
                    //#########################################################################
                    int compare =((tok_end - tok_start) == namelen || *(checkn - 1) == '.');
                        //In here the comparison that was directly passed to the if codition
                        //was pre-processed in the variable called comapre and that variable
                        //has been passed to the if condition.
                        if (compare)
                    //#########################################################################
                    {
...
//######################################################################################################
//This is method to call the while loop
size_t firstLoop(size_t tok_start, size_t no_proxy_len, const char *separator, const char *no_proxy)
{
...
//This is the method to call the nested for loop

size_t secondLoop(size_t tok_end, size_t tok_start, size_t no_proxy_len, const char *separator, const char *no_proxy)
{
    for (tok_end = tok_start; tok_end < no_proxy_len &&
                              strchr(separator, no_proxy[tok_end]) == NULL;
         ++tok_end)
    {
